...
class Retrival(BaseModel):
    db:str=Field(...,description="The name of the database")
...
async def vector_retrival(paylod:Retrival):
    try:
        retrived_data = await vector_search(databasename=paylod.db,collection=paylod.coll,query=paylod.query)
...
async def keyword_retrival(payload:Retrival):
    try:
        texts, tokens = await fetch_all_docs(database=payload.db,collection=payload.coll)
...
async def hybrid_retrival(payload:Retrival):
    try:
        hybrid_ret = await hybrid_retrieve(databasename=payload.db,collection_name=payload.coll,query=payload.query)
